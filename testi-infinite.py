...
conn = mysql.connector.connect(
    ...
    database='p3',
    user='root',
    password='PASSWORD',  # LAITA TÄHÄN SALASANAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    autocommit=True
)
...
def display_current_status():
    status_text = " Tämänhetkinen tilanteesi: \n"
    status_text += "\n Ruoat jotka sinun täytyy syödä: \n"
    for i, food in enumerate(chosen_foods, start=1):
        status_text += f"{i}. {food}\n"
    status_text += f"\n Lippuja jäljellä: {initial_tickets}\n"
    status_text += f" Rahaa jäljellä: {money}\n"
    status_text += f" Nostalgisia ruokia syöty {len(nostalgic_foods_eaten)} \n"
    return status_text
...
while initial_tickets > 0:
    ...
    if choice == "1":
        ...
        print(bordered(display_current_status()))
